...
function makeCacheKey(stationId, period) {
  return `https://weatherbear-cache.local/api/ocean-wave-height/${encodeURIComponent(
    stationId
...
async function getCachedResponse(context, stationId, period) {
  ...
  const freshData = await fetchFreshOceanWaveHeight(stationId, period);
...
async function fetchFreshOceanWaveHeight(stationId, period) {
  const url =
